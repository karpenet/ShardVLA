...
- Download and load the pretrained GR00T model via GR00TN15Backbone.from_pretrained
...
from models.gr00t.groot_n1_action_head import GR00TN15ActionHead
from lerobot.policies.pretrained import PreTrainedPolicy


class GrootBackbonePolicy(PreTrainedPolicy):
    ...
    def _create_groot_model(self):
        ...
        1) Download and load pretrained model via GR00TN15Backbone.from_pretrained
        2) Align action horizon with data_config if provided
        """
        # Handle Flash Attention compatibility issues
        self._handle_flash_attention_compatibility()

        model = GR00TN15Backbone.from_pretrained(
...
class GrootActionHeadPolicy(PreTrainedPolicy):
    """Wrapper around external Groot model for LeRobot integration."""
    ...
    def _create_groot_model(self):
        ...
        Steps (delegating to Isaac-GR00T):
        1) Download and load pretrained model via GR00TN15ActionHead.from_pretrained
        2) Align action horizon with data_config if provided
        """
        # Handle Flash Attention compatibility issues
        self._handle_flash_attention_compatibility()

        model = GR00TN15ActionHead.from_pretrained(
            pretrained_model_name_or_path=self.config.base_model_path,
